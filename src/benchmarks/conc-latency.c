 ...
 * using the c_cowg backend (C + COWG).
 * Do not modify this file; modify /Users/xin/macworkspace/cowg/src/translator/latency.ncptl instead.
 *
...
/* Header files specific to the c_cowg backend */
#include "cowg_util.h"
#include <mpi.h>
#include <stdarg.h>
...
/* Global variables specific to the c_cowg backend */
...
NULL
 ...
 /* Variables specific to the c_cowg backend */
...
break;
 ...
 /* Perform various initializations specific to the c_cowg backend. */
 /* Initialize MPI. */
...
(void) atexit (conc_exit_handler);
 ...
 /* Initialize the communication routines needed by the c_cowg backend. */
...
touchedqueue = ncptl_queue_init (sizeof (ncptl_int));
 ...
 /* Perform initializations specific to the c_cowg backend. */
...
ncptl_int j;   /* Iterate over repetitions. */
 ...
 /* Declarations specific to the c_cowg backend */
...
default:
 /* The c_generic backend or the c_cowg backend must be broken. */
...
ncptl_finalize();
 ...
 /* Finalization code specific to the c_cowg backend */
mpiresult = 0;
COWG_MPI_Finalize();
mpi_is_running = 0;
...
struct cowg_conceptual_bench latency_bench = 
{
