 ...
 * Do not modify this file; modify /Users/xin/macworkspace/codes-dev/codes/scripts/conceptual_benchmarks/allgather.ncptl instead.
 *
...
arguments[1].defaultvalue.intval = 1LL;
...
subcomm = define_MPI_communicator(procflags);
  ...
  * and volume of messages.  If so, then we can use the faster CODES_MPI_Alltoall()
  * function for the multicast instead of the slower CODES_MPI_Alltoallv(). */
{
ncptl_int msgtally = -1;   /* Messages sent or received by any participant */
ncptl_int i;
mpi_func = CONC_MCAST_MPI_ALLTOALL;  /* Use MPI_Alltoall() unless we require CODES_MPI_Alltoallv(). */
for (i=0; i<var_num_tasks; i++) {
...
if (numsenders == 1) {
 /* As a special case, use CODES_MPI_Bcast() if there's a single sender. */
...
thisev->s.mcast.size = (var_numwords)*4LL;
...
ncptl_fatal ("The c_mpi backend does not support nonzero tags in MULTICAST statements");
  ...
  * Allocate memory for CODES_MPI_Alltoallv() to use. */
...
sndvol[peervar] = (int) ((var_numwords)*4LL);
snddisp[peervar] = (int) (sndnum * ((var_numwords)*4LL));
sndnum++;
}
}
...
rcvvol[peervar] = (int) ((var_numwords)*4LL);
rcvdisp[peervar] = (int) (rcvnum * ((var_numwords)*4LL));
rcvnum++;
}
}

 /* CODES_MPI_Alltoall() includes sends to self.  Adjust sndnum and rcvnum appropriately. */
if (mpi_func == CONC_MCAST_MPI_ALLTOALL) {
CODES_MPI_Comm_size (subcomm, &sndnum);
rcvnum = sndnum;
}
...
if (1) {
CONC_EVENT *thisev = conc_allocate_event (EV_MCAST);
thisev->s.mcast.size = sndnum * ((var_numwords)*4LL);
thisev->s.mcast.alignment = 0LL;
thisev->s.mcast.misaligned = 0;
thisev->s.mcast.touching = 0;
thisev->s.mcast.verification = 0;
...
thisev->s.mcast.size2 = rcvnum * ((var_numwords)*4LL);
...
if (mpi_func == CONC_MCAST_MPI_ALLTOALL)
thisev->s.mcast.sndvol[0] = thisev->s.mcast.rcvvol[0] = (var_numwords)*4LL;
}
}
}
ncptl_free(procflags);
...
(void) conc_allocate_event (EV_RESET);
 ...
 /* THEN... */
 /* FOR 100LL REPETITIONS... */
{
ncptl_int loopnum;   /* 0=warmup repetitions; 1=main repetitions */

for (loopnum=0LL; loopnum<2LL; loopnum++) {
ncptl_int numreps = loopnum==0LL ? (3LL) : (100LL);   /* Total # of repetitions to perform */
...
ncptl_int on_event = -1;   /* Event ID of the "suppression on" event */

 /* Suppress output at the start of the warmup repetitions. */
if (numreps > 0LL && loopnum == 0LL) {
CONC_EVENT * thisev_on;   /* Output-suppression event */
thisev_on =  conc_allocate_event (EV_SUPPRESS);
...
if (unroll_loop || numreps > 0LL) {
 /* ALL TASKS BACKEND EXECUTES ['"\\n     MPI_Allgather([MESSAGE BUFFER 0], (int)"', 'var_numwords', '", MPI_INT,\\n                   [MESSAGE BUFFER 1], (int)"', 'var_numwords', '", MPI_INT,\\n                   MPI_COMM_WORLD);\\n   "'] */
{   /* ALL TASKS */
 /* The current coNCePTuaL statement applies to our task. */
...
if (numreps > 0LL && loopnum == 0LL) {
CONC_EVENT * thisev_off;   /* Output un-suppression event */

thisev_off =  conc_allocate_event (EV_SUPPRESS);
...
case CONC_MCAST_MPI_BCAST:
 /* One to many */
(void) CODES_MPI_Bcast (CONC_GETBUFPTR(mcast), thisev->s.mcast.size, MPI_BYTE,
thisev->s.mcast.root, thisev->s.mcast.communicator);
break;
case CONC_MCAST_MPI_ALLTOALL:
 /* Many to many, same to each */
(void) CODES_MPI_Alltoall (CONC_GETBUFPTR(mcast), thisev->s.mcast.sndvol[0], MPI_BYTE,
...
(void) CODES_MPI_Alltoallv (CONC_GETBUFPTR(mcast), thisev->s.mcast.sndvol, thisev->s.mcast.snddisp, MPI_BYTE,
...
suppress_output = 1;
thisev->s.suppress.stop_elapsed_usecs = ncptl_time();
}
else {
 /* Restore suppression to its previous state. */
CONC_SUPPRESS_EVENT * matchev = &eventlist[thisev->s.suppress.matching_event].s.suppress;   /* Matching suppression event */
suppress_output = matchev->prev_quiet;
if (!suppress_output) {
starttime += ncptl_time() - matchev->stop_elapsed_usecs;
}
}
break;
 ...
 /* ALL TASKS BACKEND EXECUTES ['"\\n     MPI_Allgather([MESSAGE BUFFER 0], (int)"', 'var_numwords', '", MPI_INT,\\n                   [MESSAGE BUFFER 1], (int)"', 'var_numwords', '", MPI_INT,\\n                   MPI_COMM_WORLD);\\n   "'] */
{
     ...
     MPI_Allgather((ncptl_get_message_buffer ((ncptl_int)(0))), (int) (double)thisev->s.code.var_numwords , MPI_INT,
                   (ncptl_get_message_buffer ((ncptl_int)(1))), (int) (double)thisev->s.code.var_numwords , MPI_INT,
                   MPI_COMM_WORLD);
...
uint64_t stop_elapsed_usecs = ncptl_time();
var_elapsed_usecs = stop_elapsed_usecs - starttime;
//ncptl_log_write (logstate, 0, "Gather time (us)", NCPTL_FUNC_NO_AGGREGATE, 0.0, ((double)var_elapsed_usecs)/(100.0));
starttime += ncptl_time() - stop_elapsed_usecs;
}
break;
...
static int allgather_main (int argc, char *argv[])
{
 /* Declare variables needed by all C-based backends. */
CONC_EVENT * eventlist;   /* List of events to execute */
ncptl_int numevents;   /* Number of entries in eventlist[] */
...
struct cowg_conceptual_bench allgather_bench = 
{
.program_name = "allgather",
.conceptual_main = allgather_main,
};
